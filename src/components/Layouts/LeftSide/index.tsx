    ...
    return (
        <div className={`bg-slate-100 dark:bg-slate-900 border-r border-slate-50 dark:border-slate-800 ${variant === "mobile" ? "w-auto" : "w-96"} h-app overflow-auto`}>
            ...
            <div className="px-3 py-1 sticky top-0 w-full z-20 bg-slate-100 dark:bg-slate-900">
                ...
                <Button size="sm" className="text-slate-100 gap-2 w-full" onClick={() => setShowNewChat(true)}>
            ...
            <div className="mt-5">
                {chatsFiltered.map(chat => (
                    <div key={chat.id} className={`flex items-center gap-4 py-4 px-3 ${chat.id === currentChat?.id ? 'bg-slate-200 dark:bg-slate-800' : ''} hover:bg-slate-200 hover:dark:bg-slate-700 cursor-pointer transition`} onClick={() => setChat(chat)}>
